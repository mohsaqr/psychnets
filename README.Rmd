...
Validation uses simulated data with known generating models and published psychometric datasets. Agreement with reference packages is assessed using method-appropriate criteria: exact or near-numerical agreement where objective and solver conventions coincide, and support, sign, or structural agreement where penalty paths or diagonal conventions differ. For the EBIC graphical lasso, the comparison limit is determined by the convergence criterion of the glasso implementation used by qgraph, approximately (10^{-4}). psychnets solves its native optimization to tighter tolerances. If post-estimation thresholding changes a returned graph, `certificate()` reports `NA` for that graph and retains the fitted model's residual as `$fit_kkt`.
...
Each model is fitted by `psychnets` and its reference package on the same data.
The table reports the observed comparison criterion; it is not a claim that
every independent implementation returns numerically identical weights.
