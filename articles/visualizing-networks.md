...
A psychometric network is estimated from a finite sample, so every edge
weight and every centrality value carries sampling uncertainty. The
diagnostic verbs in `psychnets` quantify that uncertainty by resampling,
case-dropping, and permutation. Each verb returns a tidy result object,
and each result object carries a base-`graphics`
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) method that
turns the numbers into a figure. The plots are drawn with the `graphics`
and `grDevices` engines that ship with R, so reading a diagnostic needs
no additional package.

Every figure in this vignette is the
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) method for one
result class. The pattern is uniform: fit a network, call a diagnostic
verb, then call [`plot()`](https://rdrr.io/r/graphics/plot.default.html)
on what the verb returns. The sections below cover the centrality plot,
the bootstrap plots and their difference-box matrices, the pairwise
difference test, the case-dropping stability curves, and the network
comparison test.
