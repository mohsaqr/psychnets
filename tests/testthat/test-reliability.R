# net_casedrop_reliability() and net_split_reliability(): edge-weight robustness
...
test_that("net_casedrop_reliability returns a tidy data.frame", {
  skip_slow()
  cd <- net_casedrop_reliability(mk(), drop_prop = c(0.3, 0.5, 0.7), iter = 20)
...
test_that("net_casedrop_reliability CS is non-increasing-ish and in range", {
  skip_slow()
  cd <- net_casedrop_reliability(mk(), drop_prop = seq(0.2, 0.8, 0.2), iter = 25)
  corr <- cd[cd$metric == "correlation", ]
  corr <- corr[order(corr$drop_prop), ]
  expect_true(corr$mean[1] + 0.05 >= corr$mean[nrow(corr)])
})

test_that("net_split_reliability returns a tidy per-metric data.frame", {
  skip_slow()
  rel <- net_split_reliability(mk(), iter = 40)
...
test_that("both verbs are estimator-agnostic (pcor) and route through psychnet", {
  skip_slow()
  cd <- net_casedrop_reliability(mk(), method = "pcor",
                             drop_prop = c(0.3, 0.6), iter = 15)
  rel <- net_split_reliability(mk(), method = "pcor", iter = 20)
  expect_s3_class(cd, "psychnet_casedrop")
  expect_s3_class(rel, "psychnet_reliability")
})
...
test_that("plot methods render without error", {
  skip_slow()
  cd <- net_casedrop_reliability(mk(), drop_prop = c(0.3, 0.5, 0.7), iter = 20)
  rel <- net_split_reliability(mk(), iter = 30)
...
test_that("print shows the CS header and the tidy table", {
  skip_slow()
  cd <- net_casedrop_reliability(mk(), drop_prop = 0.5, iter = 10)
  rel <- net_split_reliability(mk(), iter = 10)
